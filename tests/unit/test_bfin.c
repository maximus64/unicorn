...
static void test_bfin_lr(void)
{
    char code[] = "\x18\x23"; // lr %r2, %r3
    uint32_t r_pc, r_r2, r_r3 = 0x114514;
    uc_engine *uc;

    uc_common_setup(&uc, UC_ARCH_BFIN, UC_MODE_LITTLE_ENDIAN, code,
                    sizeof(code) - 1);

    OK(uc_reg_write(uc, UC_BFIN_REG_R3, &r_r3));

    OK(uc_emu_start(uc, code_start, code_start + sizeof(code) - 1, 0, 0));

    OK(uc_reg_read(uc, UC_BFIN_REG_R2, &r_r2));
...
TEST_LIST = {{"test_bfin_lr", test_bfin_lr}, {NULL, NULL}};
